...
public class Upravlenie : MonoBehaviour
{
    ...
    void Update()
    {
       ...
       // Получаем ввод пользователя
        ...
        // Создаем вектор движения
        Vector3 movement = new Vector3(moveX, 0f, moveZ).normalized; // Нормализуем вектор, чтобы скорость была постоянной


        if (Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D))
        {
            // Если движение есть, устанавливаем скорость
            if (movement != Vector3.zero)
            {
                rb.drag = 0; // Установите drag в 0, когда персонаж движется
        ...
        {
            rb.drag = Mathf.Infinity; // Увеличьте drag для быстрого торможения
            rb.velocity = new Vector3(0, rb.velocity.y, 0); // Если нет движения, устанавливаем скорость в 0
        }
        ...
        // Обработка прыжка
        if (isGrounded && Input.GetKeyDown(KeyCode.Space)) // Используем пробел для прыжка
        {
            rb.velocity = new Vector3(rb.velocity.x, jumpForce, rb.velocity.z);
        }
